...
const compile = async (unicodeCodePoints) => {
    ...
    for(let i=0; i<functions.length; i++)
    {
        ...
        {
            functionLogic(String.fromCharCode(109+i), functions[i])
        }else
        {
            functionIntLogic(String.fromCharCode(109+i), functions[i])
        }
    }
    ...
    console.log('Smart contract deployed at:', deployedContract.options.address);
}