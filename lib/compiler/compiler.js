...
var currentMemoryVariableDeclaration
var tokens
var functions
...
function nextMemoryVariableDeclaration()
{
  currentMemoryVariableDeclaration += 1
}

const compile = async (unicodeCodePoints) => {
    currentToken = 0
    currentJumpDestination = 0
    currentMemoryVariableDeclaration = 0
    ...
    for(let i=0; i<functions.length; i++)
    {
        if(functions[i].returnType == "string") // TODO: unite string and uint returns
        {
            functionLogic(functions[i].jumpDestination, functions[i])
        }else
        {
            functionIntLogic(functions[i].jumpDestination, functions[i])
        }
    }
    ...
    const contractBytecode = contractHeader(contractBodySize) + contractBody
