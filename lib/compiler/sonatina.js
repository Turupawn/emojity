...
function createConstructor() {
  ...
  blocks.set(0, {labelValues: new Map(), optimizableVariables: new Map(),  dominantBlock: null})
  let sonatinaCodeAdded = ""
...
function compileBlock(instructions, currentBlock, /* optional */ parrentBlock) {
  if(parrentBlock != null)
    blocks.set(currentBlock, {labelValues: new Map(), optimizableVariables: new Map(),  dominantBlock: parrentBlock})
  ...
  for(let i=0; i<instructions.length; i++) {
    ...
    }else if(instructions[i].name == "returnLabel")
    {
      ...
      compiledSonatinaInstructions += `    return ${typeof returnValueLoad.value != 'bigint' ? 'v' : ''}${returnValueLoad.value}.${returnValueLoad.type};\n`
    ...
    }else if(instructions[i].name == "ifStatement")
    {
      ...
      blocks.set(mergeBlock, {labelValues: new Map(), optimizableVariables: new Map(),  dominantBlock: currentBlock})
    ...
    }else if(instructions[i].name == "whileLoop")
    {
      ...
      blocks.set(conditionBlock, {labelValues: new Map(), optimizableVariables: new Map(),  dominantBlock: currentBlock})
      ...
      for (const optimizableVariable of optimizableVariablesLabels) {
        ...
        compiledSonatinaInstructions += `    evm_sstore ${typeof optimizableVariableValue.value != 'bigint' ? 'v' : ''}${optimizableVariableValue.value}.${optimizableVariableValue.type};\n`
      }
    ...
    }else if(instructions[i].name == "externalCall")
    {
      ...
      compiledSonatinaInstructions += `    br v${callResultValue}.i256 block${successBlock} block${revertBlock};\n`
      ...
      blocks.set(successBlock, {labelValues: new Map(), optimizableVariables: new Map(),  dominantBlock: currentBlock})
