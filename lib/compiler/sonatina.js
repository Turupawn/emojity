  ...
  for(let i=0; i<instructions.length; i++) {
    if(instructions[i].name == "operation")
    {
      ...
      if(stateVariables.has(instructions[i].lValue[0])) {
        let value = nextValue()
        let type = stateVariables.get(instructions[i].lValue[0]).type
        compiledSonatinaInstructions += `    v${value} = ${operator} ${sonatinaRLValue} ${sonatinaRRValue}\n`
    ...
    }else if(instructions[i].name == "assignment")
    {
      ...
      if(stateVariables.has(instructions[i].rValue[0])) {
        ...
        compiledSonatinaInstructions += `    v${value}.${type} = evm_mload(${memoryVariables.get(instructions[i].rValue[0]).location})\n`
        sonatinaRValue += `v${value}.${type}`
      } else {
      ...
      if(stateVariables.has(instructions[i].lValue[0])) {
        ...
        compiledSonatinaInstructions += `    evm_mstore(${memoryVariables.get(instructions[i].lValue[0]).location}.${type} ${sonatinaRValue})\n`
      } else {
        let value = nextValue()
        let type = labelTypes.get(instructions[i].lValue[0])
        blocks.get(currentBlock).labelValues.set(instructions[i].lValue[0], {value: value, type: type})
