...
function inheritValueFromOriginBlock(label, currentBlock, parrentBlock) {
  ...
  {
    if(parrentBlock == ssaVariableArray[i].block) {
      ssaVariables.get(label).values.push({block: currentBlock, v: ssaVariableArray[i].v})
...
function naivelyReloadAllVariables(currentBlock, parrentBlock) {
  for (const [label, value] of ssaVariables.entries()) {
    inheritValueFromOriginBlock(label, currentBlock, parrentBlock)
  }
}
...
function compileSonatinaInstructions(instructions, currentBlock, /* optional */ parrentBlock) {
  ...
  {
    naivelyReloadAllVariables(currentBlock, parrentBlock)
  }
  for(let i=0; i<instructions.length; i++) {
    ...
    }else if(instructions[i].name == "ifStatement")
    {
      ...
      naivelyReloadAllVariables(mergeBlock, currentBlock)
      ...
      for (const [label, value] of ssaVariables.entries()) {
        ...
        addValueToSSA(label, mergeBlock)
        lValue = labelToSSAVariable(label, mergeBlock)
        compiledSonatinaInstructions += `    ${lValue} = phi (${phiIfTrue} block${ifTrueReturningBlock}) (${phiIfFalse} block${currentBlock})\n`
      }
    ...
    }else if(instructions[i].name == "whileLoop")
    {
      ...
      compiledSonatinaInstructions += compileSonatinaInstructions(instructions[i].instructions, bodyBlock, currentBlock)
