...
function loadRValue(rvalue, block) {
  ...
  if(rvalue.length > 1) {
    ...
    if(blocks.get(blocks.get(block).dominantBlock).optimizableVariables.has(rvalue[0])) {
...
function storeLValue(lvalueLabel, rvalue, block) {
  ...
  if(stateVariables.has(lvalueLabel[0])) {
    if(lvalueLabel.length > 1) {
      ...
      if(blocks.get(blocks.get(block).dominantBlock).optimizableVariables.has(lvalueLabel[0])) {
...
function checkSSTOREOptimizations(instructions) {
  ...
  console.log(optimizableVariables)
