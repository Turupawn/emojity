...
function loadRValue(value, block) {
  ...
  if(value.length > 1) {
    let mappingPositionValue = getMappingPositionValue(value[0], value[1], block)
...
function getMappingPositionValue(mappingLabel, keyLabel, block) {
  ...
  let returnValue = `v${nextValue()}.i256`
  ...
  sonatinaCodeAdded += `    evm_mstore v${keccackOffsetValue}.i256 ${mappingKeyLoadValue.value};\n`
  sonatinaCodeAdded += `    evm_mstore v${keccackKeyValue}.i256 $${stateVariables.get(mappingLabel).description};\n`
  sonatinaCodeAdded += `    ${returnValue} = evm_keccak256 v${keccackOffsetValue}.i256 64.i256;\n`
  return {value: returnValue, sonatinaCodeAdded: sonatinaCodeAdded}
}
  ...
  for(let i=0; i<instructions.length; i++) {
    if(instructions[i].name == "operation")
    {
      ...
      if(stateVariables.has(instructions[i].lValue[0])) {
        if(instructions[i].lValue.length > 1){
          let value = nextValue()
          let type = "i256"
          let mappingPositionValue = getMappingPositionValue(instructions[i].lValue[0], instructions[i].lValue[1], currentBlock)
          compiledSonatinaInstructions += mappingPositionValue.sonatinaCodeAdded
          compiledSonatinaInstructions += `    v${value}.${type} = ${operator} ${sonatinaRLValue} ${sonatinaRRValue};\n`
          compiledSonatinaInstructions += `    evm_sstore ${mappingPositionValue.value} v${value}.${type};\n`
        } else {
          let value = nextValue()
    ...
    }else if(instructions[i].name == "assignment")
    {
      ...
      if(stateVariables.has(instructions[i].lValue[0])) {
        if(instructions[i].lValue.length > 1) {
          let mappingPositionValue = getMappingPositionValue(instructions[i].lValue[0], instructions[i].lValue[1], currentBlock)
          compiledSonatinaInstructions += mappingPositionValue.sonatinaCodeAdded
          compiledSonatinaInstructions += `    evm_sstore ${mappingPositionValue.value} ${sonatinaRValue};\n`
        } else {
      ...
      // TODO: Refactor lvalue load
       if(stateVariables.has(instructions[i].lValue[0])) {
        if(instructions[i].lValue.length > 1) {
          ...
          let mappingPositionValue = getMappingPositionValue(instructions[i].lValue[0], instructions[i].lValue[1], currentBlock)
          compiledSonatinaInstructions += mappingPositionValue.sonatinaCodeAdded
          compiledSonatinaInstructions += `    evm_sstore ${mappingPositionValue.value} ${instructions[i].rValue}.${type};\n`
        } else {
          let type = solidityTypeToSonatinaType(stateVariables.get(instructions[i].lValue[0]).type)
