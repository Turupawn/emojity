...
function loadValue(value, type, block) {
  let returnValue = null
  let sonatinaCodeAdded = null
  if(type == "literal") {
    returnValue = `${value}.i256`
  } else if(stateVariables.has(value)) {
    let type = solidityTypeToSonatinaType(stateVariables.get(value).type)
    returnValue = `v${nextValue()}.${type}`
    sonatinaCodeAdded = `    ${returnValue} = evm_sload ${stateVariables.get(value).position};\n`
  } else {
    returnValue = labelToValue(value, block)
  }
...
function loadRValue(value, block) {
  ...
  if(typeof value == "bigint") {
    ...
    returnValue = `v${nextValue()}.${type}`
    sonatinaCodeAdded = `    ${returnValue} = evm_sload ${stateVariables.get(value[0]).position};\n`
  } else {
...
function compileBlock(instructions, currentBlock, /* optional */ parrentBlock) {
  ...
  for(let i=0; i<instructions.length; i++) {
    ...
    {
      let gasLoadedValue = loadValue(instructions[i].gas.value, instructions[i].gas.type, currentBlock)
      ...
      let contractAddressLoadedValue = loadValue(instructions[i].contractAddress.value, instructions[i].contractAddress.type, currentBlock)
      ...
      let callValueLoadedValue = loadValue(instructions[i].value.value, instructions[i].value.type, currentBlock)
      ...
      for(let j=0; j<instructions[i].params.length; j++) {
        let paramLoad = loadValue(instructions[i].params[j].value[0], instructions[i].params[j].type, currentBlock)
