...
function convertInstructionToBytecode(instructionsParam) {
  for(let i=0; i<instructionsParam.length; i++)
  {
    ...
    {
      memoryVariablePosition = currentMemoryVariableDeclaration * 32
      nextMemoryVariableDeclaration()
    ...
    {
      let hexSelector = intToHex(parseInt(instructionsParam[i].selector))
      hexSelector = hexSelector.padEnd(64, '0')
      addPush(hexSelector)
      addPush("00")
      addOpcode("MSTORE")
      ...
      {
        let paramHex = intToHex(BigInt(instructionsParam[i].params[j]))
        paramHex = paramHex.padStart(64, '0')
        addPush(paramHex)
        addPush(intToHex(4+32*j))
        addOpcode("MSTORE")
      }
      ...
      let contractAddress = intToHex(BigInt(instructionsParam[i].contractAddress))
      addPush(contractAddress)
      let gas = intToHex(BigInt(instructionsParam[i].gas))
      addPush(gas)//gas
      addOpcode("CALL")
    }
  }
