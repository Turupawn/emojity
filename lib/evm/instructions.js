...
function putValueOnStack(label, performSizeAdjustement) {
  ...
  }else if(label.length == 2)
  {
    ...
    putMappingValueOnStack(mapLocation, label[1], 20, performSizeAdjustement) // currently only address arrays
    return
...
function putValueOnState(label, performSizeAdjustement) {
  ...
  {
    let mapLocation = intToHex(stateVariables.get(label[0]).position)
    putMappingValueOnState(mapLocation, label[1], 20, performSizeAdjustement) // TODO: Currently on array mappins
    return
  }else if(label.length == 3)
  {
    let mapLocation = intToHex(stateVariables.get(label[0]).position)
...
function logEvent(topics)
{
  ...
  if(Array.isArray(topic2))
    putValueOnStack(topic2, false)
  else
    addPush(intToHex(parseInt(topic2)))

  if(Array.isArray(topic1))
    putValueOnStack(topic1, false)
  else
