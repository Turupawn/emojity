...
function putMappingValueOnStack(mapLocation, keyLabel, performSizeAdjustement) {
  ...
  let memoryLocation = allocateMemory(32)
  storeTopOfStackInMemory(memoryLocation)
...
function putMappingValueOnState(mapLocation, storedValue, performSizeAdjustement) {
  let keySize = 32;
  if(localVariables.has(storedValue))
    keySize = localVariables.get(storedValue).size
  else if(stateVariables.has(storedValue))
    keySize = stateVariables.get(storedValue).size
  if(typeof storedValue == "bigint")
    addPush(intToHex(storedValue));
  else
    putLabelOnStack(storedValue, performSizeAdjustement);
