...
function rkeccak256(offset, size) {
  addPush(size)
  addPush(offset)
  addOpcode("KECCAK256")
}
...
function putMappingValueOnStack(mapLocation, keyLabel, performSizeAdjustement) {
  ...
  rkeccak256(intToHex(memoryLocation), intToHex(keySize + 32))
  addOpcode("SLOAD")
}
...
function put2dMappingValueOnStack(mapLocation, keyLabelA, keyLabelB, performSizeAdjustement) {
  ...
  rkeccak256(intToHex(memoryLocation1), intToHex(keySizeA + keySizeB + 32))
  addOpcode("SLOAD")
}
